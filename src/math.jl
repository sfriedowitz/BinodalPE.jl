...
const z = 1.0 # counterion valence, constant and =1 initially to test modifications
...
function gworm(q, lp, dp, b)
    coil = exp(-q*lp/2.0) / (1.0 + q^2*dp*b*lp/6.0)
    rod = (1 - exp(-q*lp/2.0)) / (1.0 + q*dp*b/pi)
    return coil + rod    
end
